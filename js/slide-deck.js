...
SlideDeck.prototype.loadConfig_ = function(config) {
  ...
  if (this.config_.presenters) {
    ...
    if (presenters.length == 1) {
      ...
      var blog = p.blog? '<span>blog</span><a href="' + p.blog +
          '">' + p.blog.replace(/https?:\/\//, '') + '</a>' : '';

      var slides = p.slides? '<span>slides</span><a href="' + p.slides +
      ...
      var html2 = [slides, twitter, www, offlinefirst].join('<br>');
